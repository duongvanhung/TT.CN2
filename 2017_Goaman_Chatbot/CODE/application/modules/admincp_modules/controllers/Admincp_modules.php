		...
		if($_POST){
			if($this->model->saveManagement()){
				...
				foreach($list_group as $v){
					if($status==0){
						$pos = strpos($v->permission,','.$this->input->post('hiddenIdAdmincp').'|');
						if($pos !== false){
							$sub_str = substr($v->permission,$pos,5+strlen($this->input->post('hiddenIdAdmincp')));
						...
						if($pos !== false){
							...
							$this->db->where('id',$v->id);
							$this->db->update('admin_nqt_groups',array('permission'=>$v->permission.','.$this->input->post('hiddenIdAdmincp').'|'.$new_perm));
						}
					}
				}
				...
				foreach($list_account as $v){
					if($status==0){
						...
						if($pos !== false){
							$new_perm = substr($v->permission,$pos+2,3);
							$this->db->where('id',$v->id);
							$this->db->update('admin_nqt_users',array('permission'=>$v->permission.','.$this->input->post('hiddenIdAdmincp').'|'.$new_perm));
						}
					}
				}
