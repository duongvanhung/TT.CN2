							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									...
									<tr role="row">
										...
										<th class="sorting" onclick="sort('name<?php ($key!='') ? print '_'.$key : print '' ?>')" id="name<?php ($key!='') ? print '_'.$key : print '' ?>">Value<?php ($key!='') ? print ' - '.mb_strtoupper($key) : print '' ?></th>
										<?php } ?>
										<th class="center sorting_disabled" width="60">Icon</th>
									...
									<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
										...
										<?php
											foreach($all_lang as $key=>$val){
												$name = ($key!='') ? 'payload_'.$key : 'payload';
										?>
										<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->$name?></a></td>
										<?php } ?>
										<td class="center"><i style="font-size:30px" class="fa <?=$v->$name?>"></i></td>
