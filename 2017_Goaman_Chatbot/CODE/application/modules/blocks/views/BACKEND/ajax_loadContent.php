							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									...
									<tr role="row">
										...
										<th class="sorting_disabled" width="150">Category</th>
										<th class="sorting_disabled" width="150">Image</th>
										<?php foreach($all_lang as $key=>$val){ ?>
										<th class="sorting" onclick="sort('name<?php ($key!='') ? print '_'.$key : print '' ?>')" id="name<?php ($key!='') ? print '_'.$key : print '' ?>">Name<?php ($key!='') ? print ' - '.mb_strtoupper($key) : print '' ?></th>
										<?php } ?>
										
										<th class="center sorting" width="60" onclick="sort('status')" id="status">Status</th>
										<th class="center sorting" width="60" onclick="sort('porder')" id="status">Order</th>
								...
								<tbody>
									...
									<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
										...
										<td class="center"><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><img alt="" src="<?=resizeImage(PATH_URL.DIR_UPLOAD_BLOCKS.$v->image,150)?>" /></a></td>
										<?php
