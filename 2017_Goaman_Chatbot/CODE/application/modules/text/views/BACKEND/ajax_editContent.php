				...
				<form id="frmManagement" action="<?=PATH_URL_ADMIN.$module.'/save/'?>" method="post" enctype="multipart/form-data" class="form-horizontal form-row-seperated">
					...
					<div class="form-body">
						...
						<div class="form-group last" style="padding-bottom:0;">
							...
							<div class="col-md-9">
								<ul class="nav nav-tabs">
									<?php $flag = false; ?>
									<?php foreach ($all_lang as $key => $data) { ?>
										<li class="<?= $flag == false ? 'active' : ''; $flag = true; ?>"><a href="#<?=$data?>" data-toggle="tab" aria-expanded="true"><?=ucwords($data)?></a></li>
									<?php } ?>
								</ul>
							</div>
						</div>
						...
						<div class="tab-content">
							<?php $flag = false; ?>
							<?php
								foreach ($all_lang as $key => $data){
									$name = ($key!='') ? 'value_'.$key : 'message';
							?>
							<div class="tab-pane fade <?=$flag == false ? 'active in' : ''; $flag = true; ?>" id="<?=$id?>">
								<div class="form-group last">
									<label class="control-label col-md-3">Message<?php ($key!='') ? print ' ('.mb_strtoupper($key).')' : print '' ?> <span class="required" aria-required="true">*</span></label>
									<div class="col-md-9"><input data-required="1" value="<?php if(isset($result->$data)) { print $result->$data; }else{ print '';} ?>" type="text" name="data<?php ($key!='') ? print '_'.$key : print '' ?>" id="data<?php ($key!='') ? print '_'.$key : print '' ?>" class="form-control"/></div>
								</div>
							</div>
							<?php } ?>
						</div>
