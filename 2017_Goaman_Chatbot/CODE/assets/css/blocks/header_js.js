...
$(document).ready(function(){
	...
	// goto element on click menu
	ScrollMenu();
	if($(window).width() > 768){
		$(window).on('load scroll', function(e){
			if($(window).scrollTop() > 30){
				//$('.tagert-menu').addClass('blk-menu-fix');
				if($('.blk-menu-fix').hasClass('template-12-flag')){
					$('.blk-menu-fix').addClass('clor-menu-fix-12');
				}
				$('.commont-menu').css({
					'padding-top': '24px',
					'z-index':'999',
				...
				$('.blk-menu-fix').css({'height':'70px'});
				$('.blk-menu-fix').addClass('combined');
			}else{
				...
				$('.blk-menu-fix').css({'height':'81px'});
				$('.blk-menu-fix').removeClass('clor-menu-fix-12');
				$('.blk-menu-fix').removeClass('combined');
			}
		});
	}else{
		$('.blk-menu-fix').removeClass('blk-menu-fix');
	}
