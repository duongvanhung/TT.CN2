					...
					<div class="form-body">
						...
						<div class="tab-content">
							...
							<div class="tab-pane fade <?=$flag == false ? 'active in' : ''; $flag = true; ?>" id="<?=$value?>">
								...
								<div class="form-group last">
									<label class="control-label col-md-3">Order<?php ($key!='') ? print ' ('.mb_strtoupper($key).')' : print '' ?> <span class="required" aria-required="true">*</span></label>
									<div class="col-md-9"><input data-required="1" value="<?php if(isset($result->$porder)) { print $result->$porder; }else{ print '';} ?>" type="text" name="porder<?php ($key!='') ? print '_'.$key : print '' ?>Admincp" id="porder<?php ($key!='') ? print '_'.$key : print '' ?>Admincp" class="form-control"/></div>
								</div>
							</div>
							<?php } ?>
						</div>
