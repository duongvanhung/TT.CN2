							...
							<table class="table table-striped table-bordered table-hover dataTable no-footer">
								<thead>
									...
									<tr role="row">
										...
										<th class="sorting" onclick="sort('porder<?php ($key!='') ? print '_'.$key : print '' ?>')" id="porder<?php ($key!='') ? print '_'.$key : print '' ?>">Order<?php ($key!='') ? print ' - '.mb_strtoupper($key) : print '' ?></th>
										<?php } ?>
								...
								<tbody>
									...
									<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
										...
										<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->$porder?></a></td>
										<?php } ?>
